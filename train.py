...
def main():
    ...
    project="STDF-refactoring",

    # track hyperparameters and run metadata
    config={
    "learning_rate": opts_dict['train']['optim']['lr'],
    "architecture": opts_dict['train']['exp_name'],
    "iterations": opts_dict['train']['num_iter'],
    }
    )
    ...
    for current_epoch in range(start_epoch, num_epoch + 1):
        ...
        while train_data is not None:
            ...
            if (num_iter_accum % interval_print == 0) and (rank == 0):
                ...
                wandb.log({"loss": loss_item, "iteracao": num_iter_accum, "epoca": current_epoch})
                print(msg)
                log_fp.write(msg + '\n')
            ...
            if ((num_iter_accum % interval_val == 0) or \
                ...
                with torch.no_grad():
                    ...
                    while val_data is not None:
                        ...
                        def process_in_patches(model, input_data, patch_size=256):
                            ...
                            output = torch.zeros((1, 3, height, width), device=input_data.device)
                        ...
                        # Avaliação
                        batch_perf = np.mean([criterion(enhanced_data[i], gt_data[i]) for i in range(b)])
                        # display
                        pbar.set_description(
                            "{:s}: [{:.3f}] {:s}"
                ...
                wandb.log({"validation value": ave_per,"iteracao": num_iter_accum, "epoca": current_epoch})
